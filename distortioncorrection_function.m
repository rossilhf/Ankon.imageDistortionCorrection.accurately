...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%插值法2
A_output=double(A(coordhigh(1):coordlow(1),coordleft(2):coordright(2),:));%把待插值数组换成double类型（int类型计数有上限）
A_temp=zeros(length(A_output(:,1,1)),length(A_output(1,:,1)),3);%相同大小的零矩阵，也是double类型

for m=1:3
    for ii=2:length(A_output(:,1,1))-1
        for jj=2:length(A_output(1,:,1))-1
            if A_output(ii,jj,m)==0 %如果是空像素，就需要插值
                count=0; %一个计数器，记录某待插值像素所在3x3范围内有多少实际像素
                for i=-1:1
                    for j=-1:1
